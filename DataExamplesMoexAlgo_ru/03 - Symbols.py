...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    for _symbol in symbols:  # Пробегаемся по всем тикерам
        ...
        fromdate = dt.datetime.now() - dt.timedelta(minutes=120*60)  # берем данные за последние 120 часов
        ...
        # fromdate = dt.datetime.now() - dt.timedelta(minutes=60)  # берем данные за последний 1 час
        # data_loader = store.getdata(timeframe=bt.TimeFrame.Minutes, compression=1, dataname=_symbol, fromdate=fromdate, live_bars=True)
        ...
        # data_loader = store.getdata(timeframe=bt.TimeFrame.Minutes, compression=60, dataname=_symbol, fromdate=fromdate, live_bars=False)
