...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    # data_loader = store.getdata(dataname=symbol, timeframe=TimeFrame.Days, fromdate=week_ago)

    # 2. Исторические часовые бары с дожи 4-х цен (four_price_doji) за текущий год
    # data_loader = store.getdata(dataname=symbol, timeframe=TimeFrame.Minutes, compression=60, fromdate=datetime(today.year, 1, 1), todate=datetime(today.year, 12, 31), four_price_doji=True)

    # 3. Исторические 30-и минутные бары с заданной даты неделю назад до последнего бара (resample из M10)
    # data_loader = store.getdata(dataname=symbol, timeframe=TimeFrame.Minutes, compression=30, fromdate=week_ago)

    # 4. Исторические 5-и минутные бары первого часа текущей сессиИ без первой 5-и минутки (resample из M5)
    # data_loader = store.getdata(dataname=symbol, timeframe=TimeFrame.Minutes, compression=5, fromdate=datetime(today.year, today.month, today.day, 10, 5), todate=datetime(today.year, today.month, today.day, 10, 55))

    # 5. Исторические и новые минутные бары с начала сегодняшней сессии (history + live M1)
    # data_loader = store.getdata(dataname=symbol, timeframe=TimeFrame.Minutes, compression=1, fromdate=today, live_bars=True)

    # 7. Исторические и новые 10 минутные бары (history + live M10)
    # data_loader = store.getdata(dataname=symbol, timeframe=TimeFrame.Minutes, compression=10, fromdate=days_ago_5, live_bars=True)

    # 8. Исторические и новые 5 минутные бары (history + live M5 resample from M1)
    # data_loader = store.getdata(dataname=symbol, timeframe=TimeFrame.Minutes, compression=5, fromdate=days_ago_5, live_bars=True)  # Пример с ТФ M5, которого нет в данных, он получается из '1m' (resample)
    ...
    # data_loader = store.getdata(dataname=symbol, timeframe=TimeFrame.Minutes, compression=5, fromdate=days_ago_5,
    #                      live_bars=True,
    #                      super_candles=True,  # для получения свечей SuperCandles с расширенным набором характеристик
